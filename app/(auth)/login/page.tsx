...
import { Zap, ArrowRight, Loader2, Lock, Mail, AlertCircle } from 'lucide-react';
import { apiFetch } from '@/lib/api';

export default function LoginPage() {
  const router = useRouter();
  const [email, setEmail] = useState('tester@sonicstudio.ai');
  ...
  const handleGoogleSignIn = () => {
    ...
    if (!googleClientId) {
      setInfoNotice(
        'Google OAuth requires NEXT_PUBLIC_GOOGLE_CLIENT_ID in your environment variables. Please provide your Google Cloud Client ID to activate 1-click Google sign-in, or use email login below.'
      );
      return;
    }
  ...
  return (
    <div className="min-h-screen flex flex-col justify-center items-center px-4 bg-[#090A0F] relative overflow-hidden">
      {/* Background glow */}
      <div className="absolute -top-40 left-1/2 -translate-x-1/2 w-[500px] h-[300px] bg-[#00D4B4]/10 blur-[140px] rounded-full pointer-events-none" />
      ...
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-xs text-slate-400 mt-1">Sign in to your creative workspace</p>
        </div>
        ...
        <div className="glass-card p-8 rounded-2xl border border-white/5 shadow-2xl space-y-5">
          ...
          <div className="text-center text-xs text-slate-400 pt-2 border-t border-white/5">
            ...
            <Link href="/register" className="text-[#00D4B4] hover:text-[#00D4B4]/80 font-medium">
              Create one now (Free 50 Credits)
            </Link>
          </div>
        </div>
      </div>
